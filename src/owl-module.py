...
def text_generation(text):
    ...
    response_text = llm_chain.invoke({'question': text})['text']
    print(response_text)
...
def text_to_speak(text, language=Language.ENGLISH.value):
    ...
    file = "raw/raw.mp3"  # TODO: Change to .wav on LINUX
    response.save(file)
...
def wake_up_call():
    ...
    while True:
        ...
        if audio:
            ...
            if undetectable:
                text_to_speak(choice(greetings_dict.get(
                    "English")))  # f"{message} Please select which language you want to communicate?"
                break
...
if __name__ == "__main__":
    env = load_dotenv()
    response = text_generation("What do you know about social tech lab?")
    text_to_speak(response)
